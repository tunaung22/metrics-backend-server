...
public class UserService : IUserService
{
    ...
    public async Task<ResultT<List<UserDto>>> FindAllAsync(bool includeLockedUser = false)
    {
        try
        {
            if (includeLockedUser)
        ...
        {
            _logger.LogError("Failed to load Users. {msg}", ex.Message);
            return ResultT<List<UserDto>>.Fail("Failed to load Users.", ErrorType.UnexpectedError);
        }
    }
}
