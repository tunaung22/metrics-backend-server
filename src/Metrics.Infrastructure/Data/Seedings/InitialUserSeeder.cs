...
{
    public static async Task InitAsync(IServiceProvider serviceProvider, SeedUserCreateDto createDto) // InitialSeedingDataConfig initialSeedingData)
    {
        ...
        try
        {
            ...
            await seedingService.SeedSysadminUser(createDto);
        }
        catch (Exception ex)
        {
    ...
    public static async Task Init_v1_Async(IServiceProvider serviceProvider)
    {
        var seedingService = serviceProvider
                       .GetRequiredService<ISeedingService>();
